...
function universe(entry) {
  ...
  draw();
  return [title("Stock & ETF universe", "Price, trend, volume and volatility research. A match still needs a current entry, invalidation, resistance target and time review.", `${candidates.length} symbols`), el("div", { class: "note-box" }, `${sourceState(entry)}. ${textValue(entry.data_freshness)}`),
...
function currentApproach(compact = false) {
  const steps = [
    ["Find the setup and catalyst", "Use fresh price, trend, volume and primary news evidence. State what could move the trade over the next 1–5 sessions."],
    ["Define entry and invalidation", "Write a conditional entry trigger and the price or event that breaks the setup. Set the risk before choosing a size."],
    ["Map real resistance", "Use observed resistance for the profit target. Check reward against risk, spread and fees; a generated 2R level is only arithmetic."],
    ["Set the time review", "Review progress at scheduled checks and set a 1–5-session deadline. Overnight holds are allowed; a failed setup is not extended into a long-term investment."],
  ];
  return el("section", { class: `value-approach${compact ? " compact" : ""}` },
    el("div", { class: "value-approach-head" }, el("div", {}, el("p", { class: "eyebrow" }, "CURRENT APPROACH · SEPTEMBER 17, 2026"), el("h2", {}, "Trade a clear setup. Give it a deadline.")), badge("Technicals + catalysts", "teal")),
    el("p", { class: "value-intro" }, "A working window of 1–5 trading days, with overnight holds allowed. The same approach applies to MSFT, AAPL, GOOGL and TSLA reviews and to new Agentic trades. A fresh review is required before any current call."),
    compact ? el("div", { class: "value-path" }, "Setup + catalyst", el("span", { "aria-hidden": "true" }, "→"), "Entry + invalidation", el("span", { "aria-hidden": "true" }, "→"), "Resistance target", el("span", { "aria-hidden": "true" }, "→"), "Time review") : el("div", { class: "value-grid" }, steps.map(([heading, copy], index) => el("article", {}, el("span", { class: "value-step" }, `0${index + 1}`), el("h3", {}, heading), el("p", {}, copy)))),
    el("p", { class: "value-status" }, "At most one new entry per day, never a quota. Five weekday checks produce a daily action brief, prompt updates when a trade qualifies, and one afternoon reminder for an unresolved sale. Exits are independent of the entry limit. A long-term DCF is not an entry gate; material business and event risks still matter."),
    el("div", { class: "value-review-links" }, el("a", { class: "text-link", href: "research/short-term-trading.html" }, "Read the short-term trading playbook"), el("a", { class: "text-link", href: "#journal" }, "Latest research and actions"), el("a", { class: "text-link", href: "research/value-investing-review.html" }, "Historical fundamental research")));
}
...
function strategy() {
  ...
  const ruleCards = [
    ["Allocation target", "25%", "A planning target, not a reason to use available cash or force a trade."],
    ["Single-position cap", ruleValue("max_position_pct"), "Size from genuine invalidation, then check exposure and concentration."],
    ["Planned risk per trade", ruleValue("risk_per_trade_pct"), "Planned entry-to-invalidation loss. Gaps, slippage and order behavior can cause larger losses."],
    ["Combined planned risk", ruleValue("max_open_risk_pct"), "A shared budget across open positions and relevant pending entries; crypto has no extra allowance."],
    ["Total exposure cap", ruleValue("max_total_exposure_pct"), "Reconcile holdings and pending orders before a new entry. The existing exposure allowance remains in force."],
    ["Group concentration cap", ruleValue("max_sector_exposure_pct"), "Correlated exposures share this cap; the target and caps do not mandate an automatic trade."],
    ["Open-position limit", ruleValue("max_positions", ""), "The existing position-count limit remains unchanged."],
    ["New entries per day", numeric(rules.max_new_entries_per_day) ? ruleValue("max_new_entries_per_day", "") : "1", "Maximum, not a daily goal. Necessary exits are never blocked by this limit."],
  ];
  return [title("The trading approach", "Short-term technical and catalyst evidence. A defined entry, exit and review clock."), currentApproach(),
    el("section", { class: "panel" }, el("h2", {}, "What would change the decision?"), el("div", { class: "decision-grid" },
      el("article", {}, el("h3", {}, "Consider an entry"), el("p", {}, "A current setup and verified catalyst offer sufficient room to real resistance. Entry, invalidation, costs and size must fit the limits. No setup means no entry.")),
      el("article", {}, el("h3", {}, "Continue the trade"), el("p", {}, "The setup remains valid, momentum is progressing and the time window has not expired. Recheck event risk before carrying overnight.")),
      el("article", {}, el("h3", {}, "Review an exit"), el("p", {}, "Act on a breached invalidation, a failed catalyst, a planned resistance target or an expired time window. Exits do not consume or wait for an entry slot.")))),
    el("section", { class: "panel" }, el("h2", {}, "Every new plan needs five things"),
      itemList(["Dated evidence: current quote, chart session, volume and the catalyst source or event date.", "Entry: the exact condition and maximum acceptable price; no chasing beyond the plan.", "Invalidation: a support break or event that disproves the setup, with planned per-unit and total risk.", "Target: observed resistance and room to reach it, with reward/risk after costs; a formula cannot invent resistance.", "Time review: the expected 1–5 trading days, progress checkpoints and a stated deadline to exit or explicitly reassess."]),
      el("div", { class: "note-box" }, "A completed long-term DCF is not required for a new entry. Earnings, legal developments and material news can still invalidate a trade or make overnight risk unacceptable. Missing current trade evidence remains a blocker.")),
    el("div", { class: "two-col" }, el("section", { class: "panel" }, el("h2", {}, "Five checks. A clear next action."), el("p", { class: "section-note" }, "The first successful regular-session check delivers a daily action brief: act now, or wait for a specific condition. New or changed actions are sent promptly. An unresolved sale gets at most one afternoon reminder at or after 14:45 Chicago, until fulfilled, declined or withdrawn. Other unchanged checks stay in the journal."),
      el("div", { class: "schedule-list" }, array(state.data?.schedule).map((time) => el("span", { class: "schedule-time" }, time))),
      el("p", { class: "section-note" }, "Monitoring is not continuous or enabled on weekends. Overnight holds are allowed, but scheduled checks and protective orders cannot guarantee the planned loss.")),
      el("section", { class: "panel" }, el("h2", {}, "One process; different market clocks"), itemList(["Stocks and ETFs use exchange sessions; crypto daily indicators use complete UTC days.", "Crypto trades 24/7. Check venue liquidity, fees and available protection independently; it shares the same capital and risk budgets.", "Short-term review covers MSFT, AAPL, GOOGL and TSLA as well as new Agentic trades. Old value-based calls are historical until a fresh technical review.", "No borrowing, options, short selling, or leveraged/inverse products."]))),
    sectionHead("Capital and risk limits are unchanged", "25% allocation target; 30% single-position, group and total-exposure caps; 1.5% planned risk per trade and across open trades."),
    ...
    el("p", { class: "section-note" }, "Past records keep their original dates, methods and calls. Historical fundamental models remain background research, not the current entry gate. This page records a process change; it does not claim that a new scan or trading plan has been completed.")];
}

function learn() {
  const cards = [
    ["Catalyst: why this window?", "A catalyst is a dated event or verified development that could matter during the planned 1–5 trading days. State the mechanism and what would disprove it. A headline alone does not make a trade actionable.", "Dated catalyst + valid setup + room to resistance"],
    ["Invalidation: where the setup fails", "Choose the price structure or event that would disprove the trade before sizing it. Do not move that level to avoid a loss or turn a failed short-term setup into an open-ended investment.", "Planned risk per unit = entry − invalidation"],
    ["Resistance: evidence for a target", "Prior highs and visible supply areas can support a profit target. State the level and chart session. Check reward against risk after costs; a calculated 2R price is not evidence that resistance lies there.", "Reward/risk = (target − entry) ÷ (entry − invalidation)"],
    ["R: planned risk math", "One R is the entry-to-invalidation distance per unit. Gaps, slippage and execution can produce larger losses. Size must fit per-trade and combined risk budgets, then exposure caps.", "Planned dollar risk = per-unit risk × quantity"],
    ["The time review: a deadline", "A trade should make progress within its planned 1–5 sessions. Reassess a stalled trade at the deadline. Overnight holding is allowed; it does not remove event risk or imply continuous monitoring.", "Setup + target + invalidation + time review"],
    ["Relative strength: compared with what?", "Stocks and ETFs compare 20-session returns with SPY. Crypto compares 20 completed UTC-day returns with BTC. Outperformance is context, not a prediction or proof of an absolute gain.", "Relative strength = asset return − benchmark return"],
    ["Volume and trend: supporting evidence", "Relative volume and moving averages describe recorded sessions. They help test participation and direction but lag or reverse. A match still needs fresh quotes, event checks and a complete plan.", "Recorded indicators ≠ a fresh executable setup"],
    ["Overnight events: a different loss path", "Verify earnings and major event dates before entry and carrying overnight. A gap can bypass invalidation. Review the trade when the catalyst changes; a stop scenario cannot guarantee a loss cap.", "Scheduled checks ≠ continuous protection"],
    ["One entry is a ceiling", "At most one new entry per day, not one required trade. An empty watchlist is valid. Necessary exits are never blocked by the entry limit. Quiet checks remain in the journal for learning.", "No qualified setup → no entry"],
    ["Fundamentals: historical background", "Dated valuation pages preserve earlier business research. They can explain material news risks, but a long-term DCF or margin-of-safety range is not an entry gate for the short-term approach.", "Historical company context + current technical review"],
  ];
...
function openCryptoCandidate(candidate, entry) {
  ...
  const body = el("div", { class: "dialog-body" },
    ...
    textSection("Shared sizing review · pending", "Stocks, ETFs, and crypto use the same position, exposure, concentration, daily-entry, and combined planned-risk limits. Crypto does not receive an extra risk allowance. No coin quantity is implied until eligibility and the shared account review are complete."),
...
function openCandidate(candidate, entry) {
  ...
  const content = el("div", {},
    ...
    el("div", { class: "dialog-body" },
      el("div", { class: "note-box" }, isArchive(entry) || isFailure(entry) ? "Historical research only. These levels are retained for the record and are not current entry instructions." : "Dated technical research only. A new short-term plan needs a fresh trigger, structural invalidation, a target supported by real resistance and a 1–5-session time review. An older record is not a new plan."),
      ...
      el("p", { class: "section-note" }, "The recorded 2R objective is arithmetic. A new trade needs a target grounded in observed resistance, with reward checked after costs. Invalidation and the time review govern exits; actual losses can exceed planned risk.")),
