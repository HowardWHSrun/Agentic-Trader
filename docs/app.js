...
function currentApproach(compact = false) {
  ...
  return el("section", { class: `value-approach${compact ? " compact" : ""}` },
    ...
    el("p", { class: "value-status" }, "At most one new entry per day, never a quota. Exits are not blocked by the entry limit. The five weekday checks continue. A long-term DCF or margin-of-safety estimate is not an entry gate; material business and event risks still matter."),
...
function strategy() {
  ...
  return [title("The trading approach", "Short-term technical and catalyst evidence. A defined entry, exit and review clock."), currentApproach(),
    ...
    el("div", { class: "two-col" }, el("section", { class: "panel" }, el("h2", {}, "Five checks. No trading quota."), el("p", { class: "section-note" }, "Scheduled weekday times in America/Chicago. Log non-actionable checks quietly and notify only for a meaningful opportunity or required action. The journal shows checks that actually completed."),
